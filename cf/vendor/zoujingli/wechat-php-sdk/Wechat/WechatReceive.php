...
class WechatReceive extends Common {
    ...
    const MSGTYPE_SERVICE = 'transfer_customer_service';//转接多客服
     ...
     * @return $this 转到多客服
     */
    public function kefuservice(){
        $msg = array(
            'ToUserName'   => $this->getRevFrom(),
            'FromUserName' => $this->getRevTo(),
            'MsgType'      => self::MSGTYPE_SERVICE,
            'CreateTime'   => time(),
        );
        $this->Message($msg);
        return $this;
    ...
    public function getUserInfo(){
        ...
        $result = Tools::httpGet(self::USER_INFO_URL . "access_token={$this->access_token}&openid={$this->getRevFrom()}");
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                 Tools::log("WechatOauth::getUserInfo Fail.{$this->errMsg} [{$this->errCode}]", 'ERR');
                return false;
            }
            return $json;
        }
